...
var (
	...
	ubi8MinimalRegex = regexp.MustCompile(`\${?UBI8_MINIMAL}?`)
	ubi9MinimalRegex = regexp.MustCompile(`\${?UBI9_MINIMAL}?`)
	goBuilderRegex   = regexp.MustCompile(`\${?GO_BUILDER}?`)

	imageOverrides = map[*regexp.Regexp]orgRepoTag{
		ubi8MinimalRegex: {Org: "ocp", Repo: "ubi-minimal", Tag: "8"},
		ubi9MinimalRegex: {Org: "ocp", Repo: "ubi-minimal", Tag: "9"},
		goBuilderRegex:   {Org: "openshift", Repo: "release", Tag: "rhel-8-release-golang-1.22-openshift-4.17"},
	}
