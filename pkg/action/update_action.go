			...
			for branchName, b := range inConfig.Config.Branches {
				if b.Konflux != nil && b.Konflux.Enabled {
					...
					steps = append(steps, map[string]interface{}{
						...
						"run": fmt.Sprintf("gh auth setup-git && git remote add fork https://github.com/serverless-qe/%s.git && git push fork %s:%s -f && gh pr create --base %s --head %s --fill-verbose",
							r.Repo,
							localBranch,
							localBranch,
							targetBranch,
